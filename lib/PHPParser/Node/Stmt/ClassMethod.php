    ...
    public function __construct($name, array $subNodes = array(), array $attributes = array()) {
        ...
        if (($this->type & PHPParser_Node_Stmt_Class::MODIFIER_STATIC)) {
            switch (strtolower($this->name)) {
                ...
                case '__destruct':
                    throw new PHPParser_Error_Fatal(sprintf('Destructor %s() cannot be static', $this->name));
                case '__clone':
                    throw new PHPParser_Error_Fatal(sprintf('Clone method %s() cannot be static', $this->name));
                default:
                    break;
            }
        }
